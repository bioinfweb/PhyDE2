...
public class ContigReferenceDataReader extends AbstractDataElementEventReader<ContigReferenceDataModel>implements ReadWriteParameterConstants {
	private boolean isPredicate;
	private String currentSequenceID = null;
	private String currentAlignmentID = null;

	
	public ContigReferenceDataReader(AlignmentDataReader mainReader) {
	...
	public void processEvent(JPhyloIOEventReader source, JPhyloIOEvent event) throws IOException {
		switch (event.getType().getContentType()) {
			...
			case LITERAL_META:
				if (event.getType().getTopologyType().equals(EventTopologyType.START)) { 
					if (PREDICATE_HAS_CONTIG_REFERENCE.equals(event.asLiteralMetadataEvent().getPredicate().getURI())) {
						isPredicate = true;
					}
					else {
						isPredicate = false;
			...
			case LITERAL_META_CONTENT:
				if(isPredicate && (currentAlignmentID != null) && (currentSequenceID != null)) {
					getCompletedElements().put(new DataElementKey(currentAlignmentID, currentSequenceID), 
							new ContigReferenceDataModel(getMainReader().getAlignmentModelReader().getCurrentModel(), event.asLiteralMetadataContentEvent().getStringValue()));	
				}	
				break;
			default:
				break;
