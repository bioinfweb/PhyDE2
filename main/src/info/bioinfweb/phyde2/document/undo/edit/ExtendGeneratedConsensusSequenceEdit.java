...
public class ExtendGeneratedConsensusSequenceEdit extends AlignmentEdit{
	public ExtendGeneratedConsensusSequenceEdit(SingleReadContigAlignmentModel contigSequences) {
	...
	public void redo() throws CannotRedoException {
		...
		if (consensusSequenceDataModel != null) { 
			for (int column = 0; column < consensusSeqLength; column++) {
				if (((consensusSequenceAlignmnetModel.getConsensusModel().getTokenAt(consensusSequenceAlignmnetModel.getConsensusSequenceID(), column)).equals(SequenceUtils.GAP_CHAR))) {
			...
			for (int column = consensusSeqLength; (column >= consensusSeqLength) && (column < maxSequenceLength); column++ ) {
			...
			throw new InternalError("No ConsensusSequenceModel was found.");
		}
	}
	...
	public String getPresentationName() {
		...
		result.append("Consesnsus Sequence extended ");
